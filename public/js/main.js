...
function updateDate() {
  ...
  async function loadBreakingNews() {
    try {
        ...
        if (error && error.code !== 'PGRST116') { // Abaikan error jika tidak ada berita sama sekali
            throw error;
        }
        ...
        if (breakingTextElement && breakingLinkElement && data) {
            breakingTextElement.textContent = data.title;
            breakingLinkElement.href = `post.html?id=${data.id}`;
